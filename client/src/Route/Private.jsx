...
export default function RutaPrivada() {
  ...
  useEffect(() => {
    const authCheck = async () => {
      const res = await axios.get(`${apiKey}/api/usuario/usuario-auth`); // Realiza una solicitud GET a la ruta "/api/v1/auth/user-auth" para verificar la autenticación del usuario
      if (res.data.ok) {
